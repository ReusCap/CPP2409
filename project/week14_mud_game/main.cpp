...
int main() {
	...
	string user_select;
	while (!currentuser){
		cout << "유저를 선택하세요 (Magician, Warrior): ";
		cin >> user_select;

		if (user_select=="Magician"){
			currentuser = new Magician();
		}else if(user_select=="Warrior"){
	...
	while (true) { // 사용자에게 계속 입력받기 위해 무한 루프
		...
		if (user_input == "up") {
			...
			if (inMap == false) {
				...
				// 위로 이동시 HP 변화 함수
				HP_change(*currentuser, map, user_x, user_y);
				// 아이템, 적, 포션 만났을 때 메시지 출력 함수
				checkState (map, user_x , user_y );
				get_item(*currentuser, map, user_x, user_y);
			}
		}
...
int HP_change(User &user, const vector<vector<int>> &map, int user_x, int user_y){
	...
	// 예외 처리
	// 맵에 0, 1, 2, 3, 4외의 값이 있을경우 예외처리 하기
	try{
		if (posState > 4 && posState < 0){
			throw invalid_argument("0, 1, 2, 3, 4 외의 값은 맵에 표현될 수 없습니다.");
		}
	}
